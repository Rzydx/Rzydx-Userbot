...
KANGING_STR = [
    ...
    "Ini Sticker Buat gua aja lah bangsat yahaha!",
    "Gua colong ni tikel ga seneng lo?",
    "Saatnya mengambil tikel ngentot ini...",
    "Papay Tikel nya buat gua ya, tar lo gua santunin😔✋",
]
...
async def kang(args):
    ...
    if isinstance(message.media, MessageMediaPhoto):
        ...
        return await edit_delete(
            args, "**File Ga di dukung nih mas, Reply foto/GIF aja kek memek!**"
        )
    if photo:
...
async def _(event):
    ...
    if not reply:
        return await edit_delete(
            xnxx, "**Reply Stiker untuk mencuri semua pack nya dekkuh!**"
        )
        ...
        try:
            create_st = await tgbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=OWNER_ID,
                    title=pname,
                    short_name=f"ram_{un_}_V{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
            ...
            await xnxx.edit("`Lagi Bikin Pack baru dek, sabar...`")
            pack += 1
            create_st = await tgbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=OWNER_ID,
