...
async def img_sampler(event):
    xx = await edit_or_reply(event, "`Jangan Nyari Gambar Bugil kentod, bentar...`")
...
@ram_cmd(pattern="google ([\\s\\S]*)")
async def gsearch(q_event):
    ram = await edit_or_reply(q_event, "`Sedang memproses...`")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return await edit_delete(ram, f"**ERROR:**\n`{e}`", time=10)
    msg = ""
    for i in range(lim):
...
@ram_cmd(pattern="yt(audio|video( \\d{0,4})?) (.*)")
