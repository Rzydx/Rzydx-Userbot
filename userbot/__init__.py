...
with bot:
    try:
        ...
        LOGS.info(
            "Join Support Group @Diskras and Channel @RzydxProject to see the updates of userbot"
            "Don't Leave")
        quit(1)
    ...
    try:
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid or event.query.user_id in SUDO_USERS:
                ...
                reply_pop_up_alert = f"Lu Gak Di Izinin Tolol, Ini Userbot Milik Si Kontol {owner}"
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid or event.query.user_id in SUDO_USERS:
                ...
                reply_pop_up_alert = (
                    f"Lu Gak Di Izinin Tolol, Ini Userbot Milik Si Kontol {owner}"
                )
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
        ...
        LOGS.info(
            "Help Mode Inline Bot Lu Gak aktif Kontol. Gak di aktifin juga gak papa serah lu aja tod. "
            "Buat Ngaktifinnya Bikin bot di @BotFather Terus Tambahin tuh var BOT_TOKEN sama BOT_USERNAME. "
            "Lu Ke @BotFather tod terus settings bot » Pilih mode inline » Turn On. Pahamkan Tod. ")
