...
with bot:
    try:
        ...
        LOGS.info(
            "Join Support Group @Rzydx_Support and Channel @RzydxProject to see the updates of userbot"
            "Don't Leave")
        quit(1)
    ...
    try:
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid or event.query.user_id in SUDO_USERS:
                ...
                reply_pop_up_alert = f"Lu Gak diizinin Kontol, Userbot ini Milik Tuan {owner}"
                ...
                if user_id is not None:
                    try:
                        ...
                        return await event.reply(
                            "❌ **Mampus Bot ini diblokir oleh pengguna.**"
                        )
                    except Exception as e:
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid or event.query.user_id in SUDO_USERS:
                ...
                reply_pop_up_alert = (
                    f"Lu Gak diizinin Kontol, Userbot ini Milik Tuan {owner}"
                )
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
        ...
        LOGS.info(
            "Help Mode Inline Bot Lu Gak hidup kontol. Gak di aktifin juga gak papa serah lu mek. "
            "Buat Ngaktifinnya, Lu Bikin bot di @BotFather Terus Tambahin var BOT_TOKEN sama BOT_USERNAME. "
            "Pergi Ke @BotFather terus settings bot » Pilih mode inline » Turn On. Pahamkan Kontol. ")
