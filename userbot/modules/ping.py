...
@register(incoming=True, from_users=5169252959, pattern=r"^.absen$")
async def _(rzydx):
    await rzydx.reply(random.choice(absen))


@register(incoming=True, from_users=5169252959, pattern=r"^.rzydx$")
async def _(rzydx):
    await rzydx.reply(random.choice(pacar))
...
async def pingme(pong):
    ...
    user = await pong.client.get_me()
    await pong.edit(f"**✨RzydxName : [{user.first_name}](tg://user?id={user.id}) **\n📗 `%sms`" % (duration))


@rzydx_cmd(pattern="pink$")
async def redis(pong):
    """For .ping command, ping the userbot from any chat."""
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
